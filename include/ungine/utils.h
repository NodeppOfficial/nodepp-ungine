...
namespace ungine { namespace rl {
    ...
    uint& GetRenderLayer() { static uint layer; return layer; }

    void SetRenderLayer( uint layer ){ GetRenderLayer()=layer; }

    void ClearRenderLayer() /*----*/ { GetRenderLayer()=0; }
...
void BeginTextureMode( GBufferRenderTexture target, int layer ) {
    ...
    rlViewport  ( 0, target.height * layer, target.width, target.height );
    rlMatrixMode( RL_PROJECTION ); rlLoadIdentity();
    rlOrtho     ( 0, target.width, target.height, 0, 0.0f, 1.0f );
...
bool IsGBufferValid( GBufferRenderTexture target ) {
    ...
    if( !IsTextureValid( target.position ) ){ return false; }
...
void UnloadGBuffer( const GBufferRenderTexture& target ) {
    ...
    if( IsTextureValid ( target.position    )  )
...
GBufferRenderTexture LoadGBuffer( int width, int height, int depth ) {
    ...
    if( target.id>0 ) {
        ...
        rlFramebufferAttach(target.id, target.position.id, RL_ATTACHMENT_DEPTH         , RL_ATTACHMENT_RENDERBUFFER, 0);
